/**
 * 99acres content script — same shape as magicbricks reference.
 *
...
const log = createLogger("99acres");

const PORTAL = "99acres" as const;
const CARD_SELECTORS = [
  ".srpTuple_srpTupleBox",
  '[data-testid="srp-tile"]',
  '[class*="srpTuple"]',
];
const DETAIL_LINK_RE = PORTAL_LISTING_URL_RE["99acres"];

// Module state — referenced by teardown.
...
/** Cache-first fetch. Dedupes concurrent requests. Never throws. */
...
/** Find a detail-page anchor inside a card (or the card itself). */
...
function setupSearchObserver(): void {
  ...
  const collect = (): void => {
    ...
    if (cards.size === 0) {
      // Fallback: any <a> matching the listing-URL regex; treat its closest
      // article-like ancestor as the host card.
