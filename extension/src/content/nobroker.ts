/**
 * NoBroker content script — same shape as magicbricks reference.
 *
...
const log = createLogger("nobroker");

const PORTAL = "nobroker" as const;
...
// PORTAL_LISTING_URL_RE.nobroker only matches the hex /property/<id> form;
// add a numeric-fallback to mirror backend/app/parsers/nobroker.py.
const NUMERIC_FALLBACK_RE = /\/[0-9]{6,}\/?(?:[?#]|$)/;
const DETAIL_LINK_RE = PORTAL_LISTING_URL_RE.nobroker;

function detailLinkMatches(href: string): boolean {
  return DETAIL_LINK_RE.test(href) || NUMERIC_FALLBACK_RE.test(href);
}
...
function findDetailLink(card: Element): { href: string; absolute: string } | null {
  ...
  for (const a of candidates) {
    ...
    if (!raw) continue;
    if (!detailLinkMatches(raw)) continue;
  ...
  const collect = (): void => {
    ...
    if (cards.size === 0) {
      document.querySelectorAll<HTMLAnchorElement>("a[href]").forEach((a) => {
        const raw = a.getAttribute("href");
        if (!raw || !detailLinkMatches(raw)) return;
